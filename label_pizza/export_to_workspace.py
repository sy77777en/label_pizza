   ...
   for qg in question_groups:
       ...
       for question, _ in questions_data:
           ...
           if question.type == "single":
               ...
               q_data["default_option"] = question.default_option or ""
...
def export_assignments(session: Session = None, output_path: str = None):
   ...
   stmt = (
       select(User.user_id_str, Project.name, ProjectUserRole.role, 
              ProjectUserRole.user_weight, ProjectUserRole.is_archived)
   ...
   assignments = session.execute(stmt).all()
   
   assignments_data = []
   for user_name, project_name, role, user_weight, is_archived in assignments:
       assignment_data = {
           "user_name": user_name,
   ...
   print(f"Exported {len(assignments_data)} assignments to {output_path}")
   ...
   for qg in question_groups:
       ...
       for answer, user_name, project_name, video_uid in annotator_results:
           ...
           if key not in grouped_answers:
               grouped_answers[key] = {}
           grouped_answers[key][answer.question_id] = answer.answer_value
