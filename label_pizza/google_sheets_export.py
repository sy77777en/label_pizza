...
class GoogleSheetExporter:
    ...
    def _get_manual_column_positions(self, role: str, include_payment: bool) -> List[int]:
        ...
        if role == "Annotator":
            ...
            # Columns: Project Name, Schema Name, Video Count, User %, All %, Last Modified, [Payment/Feedback]
            if include_payment:
                positions = [6, 7, 8]  # Payment Time, Base Salary, Bonus Salary
            else:
                positions = [6]  # Feedback
    ...
    def _create_user_sheet_headers(self, worksheet, role: str, include_payment: bool):
        ...
        if role == "Annotator":
            ...
            # Row 2 has sub-headers for Modified Ratio By
            row2 = ["", "", "", "User %", "All %", ""]  # First 6 columns
            if include_payment:
    ...
    def _create_project_row(self, project: Dict, role: str, include_payment: bool) -> List:
        ...
        if role == "Annotator":
            ...
            # Meta-Reviewer: No overall stats to avoid duplication
    ...
    def _apply_user_sheet_formatting(self, worksheet, data_rows_count: int, role: str, include_payment: bool):
        ...
        try:
            ...
            if role == "Annotator":
                ...
                column_widths.extend([
                    {"startIndex": current_col, "endIndex": current_col + 1, "pixelSize": 80},    # D: User %
                    {"startIndex": current_col + 1, "endIndex": current_col + 2, "pixelSize": 80}, # E: All %
                    {"startIndex": current_col + 2, "endIndex": current_col + 3, "pixelSize": 132}, # F: Last Modified
                ])
                current_col += 3
            ...
            # Overall stats columns (only for Annotator and Reviewer)
                    ...
                    "addConditionalFormatRule": {
                        "rule": {
                            "ranges": [{
                                ...
                                "startColumnIndex": 0,
                                "endColumnIndex": len(column_widths)
                            }],
                            "booleanRule": {
                                "condition": {
                                    "type": "CUSTOM_FORMULA",
            ...
            # Format feedback column for better text wrapping
                    ...
                    if len(row) > 0 and row[0]:  # Has user name
                        ...
                        if sheet_key in user_sheet_ids:
                            ...
                            try:
                                ...
                                print(f"        ⚠️ Could not update hyperlink for {user_name}: {e}")
                
                print(f"      ✅ Updated hyperlinks in {tab_name}")
